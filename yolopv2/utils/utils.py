...
def show_seg_result(img, result, palette=None,is_demo=False):
    ...
    if not is_demo:
        ...
        # color_area[result[0] == 1] = [0, 255, 0] # green
...
def draw_lane(img, lane_center):
    ...
    if lane_center:
        cv2.circle(img, lane_center, 5, (0, 255, 0), -1)  # Green center point
...
def get_lane_center_2(lane_points, bottom_fraction=0.5, save_path="lane_center.yaml", frame = 2):
    ...
    # Only consider lane points in the lower X% of the image (default 30%)
    y_threshold = max_y - (bottom_fraction * max_y)
    closest_points = lane_points[lane_points[:, 0] >= y_threshold]

    if len(closest_points) == 0:
        return None  # No valid closest points

    # Compute the middle point of the closest lane section
